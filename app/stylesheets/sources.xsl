    ...
    <xsl:template match="/">
        <xsl:result-document href="bibacme-sources.csv" method="text" encoding="UTF-8">
            ...
            <xsl:for-each select=".//tei:listBibl/tei:bibl">
                ...
                <xsl:text>","</xsl:text><xsl:value-of select="min(doc('../data/editions.xml')//tei:biblStruct[@corresp=$work-id]//tei:date/@when/number(.))"/>
                <xsl:text>","</xsl:text><xsl:value-of select="tei:country"/>
                <xsl:text>","</xsl:text><xsl:value-of select="tei:bibl[@type='source']/tei:ptr/substring-after(@target,'#')"/>
                <xsl:text>","</xsl:text><xsl:value-of select="$work-id"/>
                <xsl:text>","</xsl:text><xsl:value-of select="if (tei:idno[@type='cligs']) then tei:idno[@type='cligs'] else '-'"/>
                <xsl:text>","fictional</xsl:text>
                <xsl:text>","narrative</xsl:text> 
                <xsl:text>","prose</xsl:text>
                <xsl:text>","</xsl:text><xsl:value-of select="if (tei:idno[@type='cligs']) then 'n.a.' else 'yes'"/>
                <xsl:text>","</xsl:text><xsl:value-of select="if (tei:idno[@type='cligs']) then 'yes' else 'n.a.'"/>
